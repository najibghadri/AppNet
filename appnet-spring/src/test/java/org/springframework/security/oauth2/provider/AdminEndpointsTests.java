...
public class AdminEndpointsTests {
	...
	public void testListTokensByUser() throws Exception {
		ResponseEntity<String> result = serverRunning.getForString("/appnet/oauth/clients/my-trusted-client/users/marissa/tokens");
	...
	public void testRevokeTokenByUser() throws Exception {
		...
		assertEquals(
				...
				serverRunning
						.getRestTemplate()
						.exchange(serverRunning.getUrl("/appnet/oauth/users/{user}/tokens/{token}"),
		...
		try {
			...
			ResponseEntity<String> result = serverRunning.getForString("/appnet/oauth/clients/my-client-with-registered-redirect/users/marissa/tokens", headers);
	...
	public void testClientListsTokensOfUser() throws Exception {
		ResponseEntity<String> result = serverRunning.getForString("/appnet/oauth/clients/my-client-with-registered-redirect/users/marissa/tokens");
		assertEquals(HttpStatus.OK, result.getStatusCode());
	...
	public void testCannotListTokensOfAnotherUser() throws Exception {
		try {
			serverRunning.getStatusCode("/appnet/oauth/clients/my-client-with-registered-redirect/users/foo/tokens");
			fail("Should have thrown an exception");
	...
	public void testListTokensByClient() throws Exception {
		ResponseEntity<String> result = serverRunning
				.getForString("/appnet/oauth/clients/my-client-with-registered-redirect/tokens");
		assertEquals(HttpStatus.OK, result.getStatusCode());
		assertTrue(result.getBody().contains(context.getAccessToken().getValue()));
	}
	...
	public void testUserCannotListTokensOfClient() throws Exception {
		try {
			serverRunning.getStatusCode("/appnet/oauth/clients/my-client-with-registered-redirect/tokens");
			fail("Should have thrown an exception");
		} catch (UserDeniedAuthorizationException ex) {
			// assertEquals(HttpStatus.FORBIDDEN.value(), ex.getHttpErrorCode());
			assertEquals("access_denied", ex.getOAuth2ErrorCode());
