...
public class ResourceOwnerPasswordProviderTests {
	...
	public void testUnauthenticated() throws Exception {
		...
		assertEquals(HttpStatus.UNAUTHORIZED, serverRunning.getStatusCode("/appnet/photos?format=json"));
	}

	@Test
	public void testUnauthenticatedErrorMessage() throws Exception {
		HttpHeaders headers = new HttpHeaders();
		ResponseEntity<Void> response = serverRunning.getForResponse("/appnet/photos?format=json", headers);
	...
	public void testTokenObtainedWithHeaderAuthentication() throws Exception {
		assertEquals(HttpStatus.OK, serverRunning.getStatusCode("/appnet/photos?format=json"));
	...
	public void testTokenNotGrantedIfSecretNotProvided() throws Exception {
		try {
			...
			assertTrue("Wrong header " + header, header.contains("Basic realm=\"appnet/client\""));
		}
	}
	...
	public void testUserMessageIsProtectedResource() throws Exception {
		assertEquals(HttpStatus.UNAUTHORIZED, serverRunning.getStatusCode("/appnet/photos/user/message"));
	}

	@Test
	@OAuth2ContextConfiguration(ResourceOwnerWithTrustedClient.class)
	public void testClientRoleBasedSecurity() throws Exception {
		assertEquals(HttpStatus.OK, serverRunning.getStatusCode("/appnet/photos/user/message"));
	}
	...
	public void testUnsupportedMediaType() throws Exception {
		...
		// story.
		assertEquals(HttpStatus.OK, serverRunning.getStatusCode("/appnet/photos/user/message", headers));
	}
	...
	public void testUnsupportedMediaTypeWithInvalidToken() throws Exception {
		...
		assertEquals(HttpStatus.NOT_ACCEPTABLE, serverRunning.getStatusCode("/appnet/photos/user/message", headers));
	}
	...
	public void testMissingGrantType() throws Exception {
		...
		ResponseEntity<String> response = serverRunning.postForString("/appnet/oauth/token", headers, new LinkedMultiValueMap<String, String>());
	...
	static class ResourceOwner extends ResourceOwnerPasswordResourceDetails {
		public ResourceOwner(Object target) {
			...
			setAccessTokenUri(test.serverRunning.getUrl("/appnet/oauth/token"));
		}
	}
