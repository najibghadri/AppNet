...
public class AuthorizationCodeProviderTests {
	...
	public void testUnauthenticatedAuthorizationRequestRedirectsToLogin() throws Exception {
		...
		assertEquals(serverRunning.getUrl("/appnet/login.jsp"), location);
	...
	public void testInvalidScopeInAuthorizationRequest() throws Exception {
		...
		UriBuilder uri = serverRunning.buildUri("/appnet/oauth/authorize").queryParam("response_type", "code")
	...
	public void testInsufficientScopeInResourceRequest() throws Exception {
		...
		assertNotNull(context.getAccessToken());
		try {
			serverRunning.getForString("/appnet/photos?format=json");
	...
	public void testInvalidAccessToken() throws Exception {
		...
		ResponseEntity<String> response = serverRunning.getForString("/appnet/photos?format=json", headers);
		assertEquals(HttpStatus.UNAUTHORIZED, response.getStatusCode());
	...
	private String loginAndGrabCookie() {
		...
		ResponseEntity<String> page = serverRunning.getForString("/appnet/login.jsp");
	...
	static class MyLessTrustedClient extends AuthorizationCodeResourceDetails {
		public MyLessTrustedClient(Object target) {
			...
			setUserAuthorizationUri(test.serverRunning.getUrl("/appnet/oauth/authorize"));
		}
	}
